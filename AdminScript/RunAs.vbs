...
If InStr(1,strCaption, "Windows 7", vbTextCompare) Then
	If WScript.Arguments.length =0 Then
		...
		if fName = "adm" then
			objShell.Run "\\jc1dfs2\applications\desktop\AdminScript\AdminScript.exe"
		else
			strUser = InputBox("Enter username")
			objShell.Run "runas /profile /user:knight\" & strUser & " \\jc1dfs2\applications\desktop\AdminScript\AdminScript.exe"
		end if
	end if
